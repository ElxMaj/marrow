...
- `.github/workflows/release.yml` runs typecheck, lint, tests, packed smoke, builds, and then runs `changeset publish` on a tagged release (`v*`) only, never on a normal merge.
...
4. The release workflow fails before tests if `NPM_TOKEN` is missing, so a tag cannot silently skip publishing.
...
# the release workflow verifies the package path, builds, and publishes on the tag
```
